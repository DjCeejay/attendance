...
@section('title', 'Sign In - Staff Attendance')
...
<div class="bg-white rounded-2xl shadow-xl border border-slate-200/80 p-6 md:p-8 mt-6">
    <div class="text-center mb-6">
        ...
        <p class="text-xs text-slate-500 mt-1">Use your staff account credentials</p>
    </div>
    ...
    <form method="POST" action="{{ route('login') }}" class="space-y-4">
        ...
        <div>
            ...
            <input type="email" name="email" id="email" value="{{ old('email') }}" required autofocus class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm outline-none transition">
        </div>

        <div>
            <label for="password" class="block text-xs font-bold uppercase tracking-wider text-slate-700 mb-1">Password</label>
            <input type="password" name="password" id="password" required class="w-full px-4 py-3 rounded-xl border border-slate-300 focus:ring-2 focus:ring-indigo-500 focus:border-indigo-500 text-sm outline-none transition">
        </div>
        ...
        <div class="text-center pt-3 border-t border-slate-100">
            <p class="text-xs text-slate-600">
                New staff member? 
                <a href="{{ route('register') }}" class="font-extrabold text-indigo-600 hover:text-indigo-800">Register account here</a>
            </p>
        </div>
    </form>
</div>
